...
class InvoiceController extends Controller
{
    ...
    public function index()
    {
        ...
        })->where('customer_id', auth()->guard('api_cust')->user()->id)->paginate(5);

        //return with Api Resource
        return new InvoiceResource(true, 'List Data Invoices : '.auth()->guard('api_cust')->user()->name.'', $invoices);
    }
    ...
    {
        $invoice = Invoice::with('orders.product', 'customer', 'city', 'province')->where('customer_id', auth()->guard('api_cust')->user()->id)->where('snap_token', $snap_token)->first();
